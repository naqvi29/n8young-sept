...
def profile():
    ...
    if len(r) == 0:
        app.db.add({'user_id': id_, 'balance': 0, 'upload':True})
        pb = 0
    else:
        pb = r[0]['balance']
    print(notes)
    ...
    if request.method == 'POST':
        try:  
            ...
            if allow_upload == False:
                verify = False
                verify_msg = "You can't upload new files until the previous one is finished aligning."
...
def confirm_uploads():
    ...
    if request.method == "POST":
        ...
        db_id = r[0]['id']
        app.db.updateById(db_id, {'upload': False})
